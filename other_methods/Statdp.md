...
## 手法の整理（自分の理解）
以下のような仮説検定を考える。
H0: $p_1 \leq e^\epsilon \cdot p_2$ (ε-DPを満たす)
H1: $p_1 > e^\epsilon \cdot p_2$ (ε-DPを満たさない)

この時のp値（H0が正しいと仮定した時に観測データが得られる確率）が有意水準αより小さい場合、帰無仮説が棄却され、観測結果からε-DPが満たされないことを検出できる。

よって、観測結果からp値を計算できれば良い。

ここで一般に、以下のようなFisher の Exact Test における片側検定を考える。
H0: $p_1 \leq p_2$
H1: $p_1 > p_2$

この場合のp値は以下のように計算可能である。
p値=1−Hypergeometric.cdf( $c_1−1∣n_1+n_2,n_1,c_1+c_2$ )

上記についてより詳細に解説する。

$p_1$をグループAの成功確率、$p_2$をグループBの成功確率であるとすると、p値はグループAが$c_1$以上の成功を得る確率となる。

p値とは帰無仮説を前提とした時に、どれだけおかしな（極端な）観測結果が出るかの確率であり、今回対立仮説として、 $p_1 > p_2$を設定しているため、Aが極端に多く成功するという観測結果になる事後確率を考える。

そう考えると「p値=P(Aが c1​ 以上成功 | $p_1​=p_2$​)」となる。もしこの値が低いと、前提としている $p_1 = p_2$ という帰無仮説が誤っているということになる。これが超幾何分布の累積分布関数を用いて表すことができるのは、以下のようなケースを想定するからである。なお、成功回数をsと固定して、それをA, Bに無作為に割り振っており、そこで前提としての $p_1 = p_2$を使っている（A, Bが独立という意味）。
- A/Bグループの人数: $n_1, n_2$
- 箱の中のボールの数: $n_1 + n_2$
- A/Bグループの当たりの人数: $c_1, c_2$
- 当たり玉(成功回数): $c_1 + c_2 = s$
- 観測: 観測：Aグループで成功者が $c_1$ 人以上になる確率

ただし、本来は $H_0: p_1 \leq e^\epsilon \cdot p_2$ という $e^\epsilon$ をかけたものを考えたい。

そこで、$C_1 \sim Bin(n, p_1)$でサンプリング後、再度、一回の試行（ベルヌーイ試行）の成功確率が $1/e^\epsilon$ の試行を$C_1$ 回行う。これにより、$\tilde{C_1} \sim Bin(C_1, 1/e^\epsilon)$となり、帰無仮説は以下のように扱える。
$$
\tilde{p_1} = p_1 / e^\epsilon \leq p_2
$$

疑似アルゴリズムは以下のようになる。
```
function pvalue(c1, c2, n, ε):
    c1_tilde ← Binomial(c1, 1/e^ε)    # スケーリング
    s ← c1_tilde + c2
    return 1 - Hypergeom.cdf(c1_tilde - 1 | 2n, n, s)
```
