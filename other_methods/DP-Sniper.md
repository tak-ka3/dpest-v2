...
## 手法整理（自分の理解）
ある隣接入力において、それらを区別することを最も可能にする出力集合を探す。連続的な確率分布なので、出力集合は単一の値ではなく、範囲を持った集合Sである。

まず、単一の出力を見てそれがどちらの入力であるかを区別する分類器を学習させる。これにより、p(a|b)を全般的に高めることができる。

ここで自然に考えるのは、同じ入力からたくさん出力させて、それぞれにおいてp(a|b)を求めて、上位k番目の出力を集合Sに採用することだろう。

ただ、出力は連続的な値であるので、集合Sは離散的な出力値の集合となる。よって、他の出力が集合Sに含まれることは起き得ない。

そこで、集合Sを直接求めるのではなく、P[M(a) ∈ S]やP[M(a') ∈ S]の比を最大化させるようなそれらの値を直接求めることを考える。

まず以下の仮説検定を考える。
- 帰無仮説H0: 出力bは分布P0（例：M(a′)）から生成された
- 対立仮説H1: 出力bは分布P1（例：M(a)）から生成された

一般的な仮説検定では、「帰無仮説を統計的に棄却するかどうか」を判断するものだが、この帰無仮説を最適な出力集合を構築するために用いる。

具体的にはNeyman–Pearson補題を用いる。Neyman–Pearson補題とは、「与えられた偽陽性率（false positive rate）の下で、真陽性率（true positive rate）を最大化する検定は尤度比検定（likelihood ratio test）である」というものだ。

ここでFP, TPは以下のようになる。
FP: Pr[M(a′) ∈ S]
  - 帰無仮説を棄却したが、実際は帰無仮説が正しかった確率
TP: Pr[M(a′) ∈ S]
  - 帰無仮説を棄却して、実際に帰無仮説は誤りで対立仮説が正しかった確率

よって、FP/TPまたは、TP/FPを最大化したい。

ここで、FP/TPそれぞれの確率をc以下に制限する。なぜなら、確率が小さすぎると誤差が大きくなるだ。よって、FP=Pr[M(a′) ∈ S]=cもしくはc以下の値になるように制限する。

よって、FPを最大化する必要があるが、Neyman–Pearson補題を思い出すと尤度比検定を行うことで求められる。
具体的には、尤度比 Λ(b)=P(b∣a)/P(b∣a′)が高い順に出力bを並べて、上位から false positive の合計がちょうど c になるまで含めると、true positive が最大化される。

ここでベイズの定理より、尤度比はΛ(b)はp(a|b)の単調変換で求められるので、p(b|a)を高い順に並べれば良い。

こうして、閾値tや閾値tの時の確率qが求められる。これが攻撃集合S_{t, q}である。具体的には、一般的な値の集合ではなく、以下のようなbの関数となる。
- S_{t, q}(b) = 
  - 1 if p(a|b) > t
  - q if p(a|b) = t
  - 0 if p(a|b) < t

これが最適な出力集合（攻撃集合）なので、これを用いることで、出力の比を最大化可能。
---
