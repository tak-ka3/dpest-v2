...
| サンプリング | $N=100,000$ | $N \times m = 5 \times 10^5$ |

**理論上はサンプリングが50倍有利**に見えます。
...
| サンプリング（非最適化） | $5 \times 10^5$ | 50× | $2.5 \times 10^7$ | ~100秒（推定） |
| サンプリング（ベクトル化） | $5 \times 10^5$ | 1× | $5 \times 10^5$ | ~0.1秒（推定） |

**重要な洞察**:
> **理論計算量が50倍少なくても、実装レベルの最適化により実効速度が50倍遅くなり、結果的に同等または遅くなる**
...
```python
# ベクトル化実装（可能だが未実装）
def report_noisy_max_vectorized(x, n_samples=100_000, epsilon=0.1):
    m = len(x)
    # 全サンプルを一括生成（SIMD最適化）
...
**現状（非最適化サンプリング）での解析モードの優位性**:

1. **実装レベルの最適化**: NumPy/FFTWのベクトル化・SIMD最適化により、理論計算量の50倍の差を実効速度で覆す
2. **高精度**: 決定論的誤差6.9% vs 確率的誤差8〜15%
