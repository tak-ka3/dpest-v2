...
3. ノイズ付きベクトルの **最大値** を返す: $\max_i (q_i + \eta_i)$ **数式**:
...
**全体計算量**: $O(m^2 \times g^2)$ ## 理論的な誤差（精度）
...
**結論**: DPESTはDP-Sniperより25倍、StatDPより45倍大きなプライバシー損失を検出し、プライバシー違反を最も正確に報告。
...
**結論**: DP-Sniperより7倍高速、StatDPより44倍高速で、プライバシー違反検出能力も圧倒的に優れている。
