...
3. 各累積和にLaplaceノイズを追加: $\tilde{s}_i = s_i + \text{Lap}(1/\varepsilon)$
...
**結論**:
- **無限大の検出**: DPESTのサンプリングモードでは、サンプル数不足により特定のビンで片方のデータベースのみサンプルが存在する状況が発生し、無限大のプライバシー損失が誤って検出された
- **サンプリングモードの課題**: 出力空間が離散的で広い場合（プレフィックス和の組み合わせ）、モンテカルロサンプリングでは全ての出力値を十分にカバーできず、確率比の推定が不安定になる
- **他手法との比較**: DP-SniperとStatDPは有限値（0.35-0.50）を推定
- **実行時間**: DP-Sniperより2.1倍遅いが、StatDPより16倍高速。ただし推定精度に問題がある
