...
2. **Add演算（定数+連続）**: $O(n \times g)$ - $n=20$ 回の独立なシフト操作

**実効計算量**（ $n=20$ , $g=1000$ ）: $20 \times 1000 \approx 2 \times 10^4$ 演算

## 理論的な誤差（精度）

LaplaceMechanismと同様、定数+Laplace分布の単純なシフトであり、理論的にはアルゴリズム自体の誤差はありません。観測される誤差は格子近似と数値計算による実装誤差です。
...
1. **精度**: DPESTは理論値に最も近い（誤差1.0%）
   ...
   - StatDP: 誤差86%（大きく過小評価）
2. **速度**: DP-Sniperより1500倍高速、StatDPより39000倍高速
