...
- 依存関係の有無に関わらず、すべてのアルゴリズムに適用


#### モードの自動選択

dpestは、アルゴリズムの入力から計算グラフの各オペレーションを走査し、入力の確率変数同士に依存関係があった時点でそのオペレーションノードの出力確率変数はサンプリングモードで確率分布を求めることになります。一方で、確率変数同士が互いに独立である場合は、解析モードを用いて出力の確率分布を求めます。

サンプリングモードにおいて、ユーザーは明示的にサンプル数を設定することもできます：
