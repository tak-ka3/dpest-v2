...
**Laplace分布の場合**: dpestでは、Laplace分布のサポート範囲を $[\mu - 7b, \mu + 7b]$ に設定しているため、$E_{\text{trunc}} = 2e^{-7} \approx 0.0018$ (0.18%)。

#### 1.2.2 補間誤差（Interpolation Error）
...
解析手法の計算量は演算により異なりますが、主要な演算の計算量は以下の通りです：

- **Add（連続+連続）**: $O(g \log g)$ - FFT畳み込み
- **Add（離散+連続）**: $O(kg)$ - シフト操作（$k$: アトム数）
- **Affine**: $O(g)$ - 線形変換
- **Max/Min/Argmax**: $O(n^2 g^2)$ - CDF計算（$n$: 分布数）
- **Compare**: $O(g)$ - CDF利用

典型的なパラメータ $g=1000$ では、基本演算は $10^3$ から $10^6$ 演算で実行可能です。
...
サンプリング手法は、Monte Carlo法により確率分布 $P(X)$ から $N$ 個のサンプル $\{x_1, \ldots, x_N\}$ を生成し、経験分布で近似します：
...
サンプリング手法の総計算量は、サンプル生成とヒストグラム構築の2つの部分から構成されます：

$$
T_{\text{total}} = N \times T_{\text{sample}} + O(N \cdot d + B)
$$

ここで：
- $N$: サンプル数
- $T_{\text{sample}}$: 1サンプルあたりの生成コスト（アルゴリズム依存）
- $d$: 出力の次元数
- $B = \prod_{i=1}^{d} b_i$: ヒストグラムの総ビン数

典型的なパラメータ $N=100,000$, $n=10$ では、$T_{\text{total}} = 10^6$ 演算程度です。
