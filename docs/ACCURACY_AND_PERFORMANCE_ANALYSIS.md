...
**実測値との比較**:
- LaplaceMechanism: 推定ε=0.1002, 理論ε=0.1, 誤差=0.2% ✓
- NoisyHist1: 推定ε=0.1002, 理論ε=0.1, 誤差=0.2% ✓
- NoisyHist2: 推定ε=10.0001, 理論ε=10.0, 誤差=0.001% ✓
- ReportNoisyMax1: 推定ε=0.1069, 理論ε=0.1, 誤差=6.9%

実測誤差が理論上界の0.18%よりも大きい場合（ReportNoisyMax1で6.9%）は、**複数の演算を組み合わせた際の誤差の累積**による。特にArgmax演算のような複雑な順序統計量計算では誤差が増大する傾向がある。
...
**実測**: 推定ε=0.1002, 理論ε=0.1, 誤差=0.2% ✓

**計算量**: $T = O(g \log g) = 10^4$ 演算, 実測0.01s
...
**実測**: 推定ε=0.1010, 理論ε=0.1, 誤差=1.0% ✓

**計算量**: $T = 20 \times O(g \log g) = 2 \times 10^5$ 演算, 実測0.04s
...
**計算量**: $T = 5 \times O(g \log g) = 5 \times 10^4$ 演算, 実測0.01s
...
**実測**: 推定ε=10.0001, 理論ε=10.0, 誤差=0.001% ✓

**計算量**: $T = 5 \times 10^4 + 10^6 = 1.05 \times 10^6$ 演算, 実測0.01s
...
**実測**: 推定ε=0.1069, 理論ε=0.1, 誤差=6.9%

**計算量**: $T = 5 \times 10^4 + 10 \times 10^6 = 10^7$ 演算, 実測2.75s
...
**実測**: 推定ε=0.0964, 理論ε=0.1, 誤差=-3.6% ✓

**計算量**: $T = 10^7$ 演算, 実測2.75s
...
**実測**: 推定ε=0.6005, 理論ε=0.8, 誤差=-24.9%

**計算量**: $T = O(2^k)$ (ビット列の組み合わせ、$k$=ビット数), 実測0.01s
...
**実測**: 推定ε=0.8477, 理論ε=∞, 誤差=N/A

**注**: 理論値∞に対して有限値を返すのは、解析手法の離散化による近似の影響。

**計算量**: $T \approx 10^7$ 演算, 実測2.73s
...
**実測**: 推定ε=1.8692, 理論ε=∞, 誤差=N/A

**注**: 複雑な max 演算の組み合わせにより、解析手法でも実行可能。

**計算量**: $T \approx 2 \times 10^7$ 演算, 実測24.40s
...
**計算量**: $T = N \times O(n) = 1,000,000 \times 10 = 10^7$ 演算, 実測276.45s
...
**計算量**: $T = N \times O(n) = 1,000,000 \times 10 = 10^7$ 演算, 実測47.02s
...
**計算量**: $T = 10^7$ 演算, 実測24.79s

**早期リターン最適化の効果**:
- ∞検出により残りのペアをスキップ
- 実測時間がSVT3の半分程度（24.79s vs 47.02s）
...
**計算量**: $T = 10^7$ 演算, 実測57.76s

**早期リターン最適化の顕著な効果**:
- 実測時間がSVT1の約1/5（57.76s vs 276.45s）
- ∞検出による早期終了が有効
...
|------------|----------|------|-------|-------|------|---------|------|
| LaplaceMechanism | 1 | 解析 | 0.1002 | 0.10 | 0.2% | 0.01s | 最高速 |
| LaplaceParallel | 20 | 解析 | 0.1010 | 0.10 | 1.0% | 0.04s | 並列独立 |
| NoisyHist1 | 5 | 解析 | 0.1002 | 0.10 | 0.2% | 0.01s | 精度改善 |
| NoisyHist2 | 5 | 解析 | 10.0001 | 10.00 | 0.001% | 0.01s | 精度改善 |
| ReportNoisyMax1 | 5 | 解析 | 0.1069 | 0.10 | 6.9% | 2.75s | Argmax演算 |
| ReportNoisyMax2 | 5 | 解析 | 0.0964 | 0.10 | -3.6% | 2.75s | Argmax演算 |
| ReportNoisyMax3 | 5 | 解析 | 0.8477 | ∞ | - | 2.73s | 理論∞ |
| ReportNoisyMax4 | 5 | 解析 | 8.6719 | ∞ | - | 2.74s | 理論∞ |
| OneTimeRAPPOR | 1 | 解析 | 0.6005 | 0.80 | -24.9% | 0.01s | 離散演算 |
| RAPPOR | 1 | 解析 | 0.3001 | 0.40 | -25.0% | 0.00s | 離散演算 |
| NoisyMaxSum | 20 | 解析 | 1.8692 | ∞ | - | 24.40s | 複雑な max |
| SVT1 | 10 | サンプリング | 0.0920 | 0.10 | -8.0% | 276.45s | Branch依存 |
| SVT2 | 10 | サンプリング | 0.0843 | 0.10 | -15.7% | 282.88s | Branch依存 |
| SVT3 | 10 | サンプリング | ∞ | ∞ | - | 47.02s | 早期終了 |
| SVT4 | 10 | サンプリング | 0.1761 | 0.18 | -2.2% | 310.38s | Branch依存 |
| SVT5 | 10 | サンプリング | ∞ | ∞ | - | 24.79s | 早期終了 |
| SVT6 | 10 | サンプリング | 0.4976 | ∞ | - | 278.91s | - |
| NumericalSVT | 10 | サンプリング | ∞ | 0.10 | - | 57.76s | 早期終了 |
| PrefixSum | 10 | サンプリング | ∞ | 0.10 | - | 123.84s | 累積依存 |
| SVT34Parallel | 10 | サンプリング | ∞ | ∞ | - | 105.22s | - |
| TruncatedGeometric | 5 | サンプリング | 0.1312 | 0.12 | 9.3% | 23.92s | Branch依存 |

**注記**:
- サンプル数: $N = 1,000,000$
- ヒストグラムビン数: 100
- 誤差 = (推定ε - 理論ε) / 理論ε × 100%
- NoisyHist1/2は隣接パターンを2つ（one_above, one_below）に制限することで精度が大幅に改善
- OneTimeRAPPOR, RAPPORは理論値と大きく乖離（要調査）
