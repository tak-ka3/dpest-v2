...
通常、CDFは累積和で O(g) で計算できます：
```python
cdf = np.cumsum(f) * dx  # O(g)
```

しかし、Argmaxでは**異なる格子を持つ分布同士を比較**するため：
```python
# target分布の格子
x_grid    = [-65.0, -64.86, ..., 74.86, 75.0]  # 1000点

# other分布の格子（微妙にずれている！）
dist_x    = [-64.0, -63.85, ..., 75.85, 76.0]  # 1000点
```

格子が一致しないため、各 `x_grid` の点ごとに独立して積分を計算する必要があり、累積和による高速化ができません。これがCDF計算がO(g²)になる根本原因です。
...
CDF計算がO(g²)なのは**1回の呼び出しあたり**の計算量です。しかし、CDF計算は**n²回呼ばれる**ため：

```
CDF計算1回あたり: O(g²)
呼び出し回数: n個のインデックス × (n-1)個のother分布 ≈ n²

全体の計算量: n² × g² = O(n²·g²)
```

つまり、「nの二乗」と「gの二乗」の**両方**に比例して計算時間が増加します。
