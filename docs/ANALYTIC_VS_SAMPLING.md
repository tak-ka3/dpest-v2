...
dpestフレームワークは、アルゴリズムの依存関係を自動的に検出し、適切なモード（解析/サンプリング）を選択します：

**検出ロジック** (`dpest/engine.py:107-153`):

1. **個別ノードの依存関係チェック**:
   - `branch`演算などが明示的に`needs_sampling=True`を設定

2. **リスト要素間の依存関係チェック** (重要！):
   ```python
   # SVT5/SVT6のケース: 共通のノイズ付き閾値Tに依存
   over[0].dependencies = {id(Q[0]), id(lap_T)}
   over[1].dependencies = {id(Q[1]), id(lap_T)}
   # 共通部分 {id(lap_T)} を検出 → サンプリングモード
   ```

**SVT1の依存関係**:
- `branch`演算による明示的な依存関係
- カウンタ変数の累積的更新
- 打ち切り条件による後続クエリへの影響

**SVT5/SVT6の依存関係**:
- 全クエリが同じノイズ付き閾値`T`を共有
- リスト要素間で共通の依存関係を持つ
- `branch`はないが、サンプリングモードが必要
